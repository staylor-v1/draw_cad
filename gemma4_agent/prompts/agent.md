...
- If `cad_construction_strategies` is provided, choose one primary strategy before coding and follow its construction sequence. Use it as a design-intent plan, not as decoration: a flange/hub with section diameters should be a revolved profile plus secondary features; a flat plate/link should be a sketched profile plus extrude/cuts; a machined block should be stock plus additive/subtractive features; a curved support should be swept/lofted from a path and section.
- Prefer feature history that would survive dimension edits: named dimensions, symmetry, bolt-circle patterns, mirror/pattern operations, and finishing features applied after the primary body/cuts. Avoid early freehand booleans that make radii, chamfers, holes, or internal steps hard to revise.
