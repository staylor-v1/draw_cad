...
class Gemma4RoundTripAgent:
    ...
    def run_roundtrip(
        ...
        summary = {
            "success": bool(comparison["equivalent"]),
            "output_dir": str(root),
    ...
    def _fallback_code_for_drawing(self, drawing_path: Path) -> str | None:
        ...
        if suffix in {".png", ".jpg", ".jpeg", ".webp", ".bmp"}:
            ...
            return _box_code(width, height, thickness, f"fallback from raster {drawing_path.name}")
        ...
        if suffix == ".svg":
            ...
            return _box_code(width, depth, height, f"fallback from SVG triplet {drawing_path.name}")
...
def _box_code(width: float, depth: float, height: float, reason: str) -> str:
    return f'''from build123d import *

# Conservative baseline generated by the roundtrip harness: {reason}.
# Gemma did not produce an executable model, so use the drawing envelope.
part = Box({width:.6g}, {depth:.6g}, {height:.6g})
'''
